...
export class Chlorine extends Entity{
  ...
  m_Rigidbody: Cube;
  m_ChlorineModelEntity: ChlorineModel;
  ...
  Init(): void {
    ...
    this.m_Rigidbody.m_Mesh.id = "CubeRigidbody"
  ...
  const impostor = new PhysicsImpostor(
    ...
    this.m_ChlorineModelEntity.setParent(this.m_Rigidbody.m_Mesh);
    var tmp = this.m_Scene.getTransformNodeById("Chlorine");
    ...
    this.actionManager.registerAction(new ExecuteCodeAction(
        ...
        () => {
          if (this.parent?.parent?.name === "Beaker") return;
            ...
            this.m_ChlorineModelEntity.m_Model.m_Mesh.dispose();
            this.m_TextPlane.m_Mesh.dispose();
            this.Destroy();
            this.m_ChlorineModelEntity.Destroy();
        }
      )
    );
...
export class ChlorineModel extends Entity{
